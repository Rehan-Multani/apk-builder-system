...
async function buildAPK(data, updateStatus, job) {
    const { buildId, url, appName, packageName, splashColor, splashMode, versionName, versionCode, iconPath, splashPath, storePassword, keyPassword, keyAlias, keystoreName, googleServicesPath, fcmStoreUrl, fcmBody, apiHeaders, splashDuration, useSafeArea, safeAreaTop, safeAreaBottom, usePaymentGateway, paymentGatewayType, paymentGatewayKey } = data;
    ...
    try {
        ...
        const config = {
            ...
            usePaymentGateway: usePaymentGateway !== undefined ? (usePaymentGateway === 'true' || usePaymentGateway === true) : false,
...
`;
    ...
    // 1. Try running 'flutter sdk-path' (Source of truth)
    try {
        const { stdout } = await execPromise('flutter sdk-path');
        flutterSdkPath = stdout.trim().replace(/\\/g, '/');
    } catch (e) {
        console.log("flutter sdk-path failed, trying fallback...");
    }
    
    // 2. Try locating via executable path
    if (!flutterSdkPath) {
        try {
            ...
            console.log("Could not locate flutter executable, trying environment variables...");
        }
    }
    
    // 3. Fallback to process.env.FLUTTER_ROOT
    if (!flutterSdkPath) {
        flutterSdkPath = process.env.FLUTTER_ROOT ? process.env.FLUTTER_ROOT.replace(/\\/g, '/') : '';
    }
